...
public:
	unordered_map<unsigned int, shared_ptr< C > > m_ClientList;
...
bool CServer<C>::AddClient(SOCKET socket)
{
	...
	msave_ulCurrKey = m_ulCurrKey;

	m_ClientList.insert( typename unordered_map <unsigned int, shared_ptr< C > >::value_type(m_ulCurrKey, pClient));
...
bool CServer<C>::Parser(shared_ptr< C > pClient)
{
	...
	while( (unsigned long)pClient->m_iReadBuffer >= sizeof(PACKET_HEADER) )	
	{
		...
		Trace(TR_INFO, "CServer<C>::Parser 001=>%ld %ld %s", ulSize, ulType, pClient->m_ReadBuffer.get()+sizeof(PACKET_HEADER));
		//printf("222abcde  %d  %d\n", (unsigned long)pClient->m_iReadBuffer, ulSize);

		if( (unsigned long)pClient->m_iReadBuffer < ulSize ) 
			break;

		Trace(TR_INFO, "CServer<C>::Parser 002");
...
#endif
{
	...
	// select loop
#ifdef	WIN32
	DWORD dwRet;	// for WaitForSingleObject()
	while(true)	
	{
	...
	for( R = m_ClientList.begin(); R != m_ClientList.end(); R++ )	
	{
		...
		if( !pClient->m_bRemove )	
		{
			...
			if( pClient->m_iWriteBuffer > 0 )
				FD_SET(pClient->m_socket, &writefds);
				
#ifdef	__LINUX__
			if( maxfd < pClient->m_socket )
				maxfd = pClient->m_socket;
#endif
		}
	}
...
check_timer:
	...
	if( m_CurrTime < m_LastTime ) 
		m_CurrTime = m_LastTime = 0;
...
shared_ptr<C> CServer<C>::GetClient(unsigned long ulKey)
{
	...
	typename unordered_map <unsigned int, shared_ptr<C> >::iterator I;
	shared_ptr<C> pClient;
