...
AMQPQueue::~AMQPQueue() {
	...
	if (pmessage)
		...
		printf("[%s][%s][%d][Error Exception Occurred]", __FILE__, __FUNCTION__, __LINE__);
	}
}
...
void AMQPQueue::sendConsumeCommand() {
	...
	*/
	...
	//--ischoi-- auto_ptr<AMQPMessage> message ( new AMQPMessage(this) );
	//--ischoi-- pmessage = message.get();
	pmessage = new AMQPMessage(this);
