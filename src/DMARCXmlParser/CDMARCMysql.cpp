...
CDMARCMysql::CDMARCMysql():m_iPort(3306), m_iRetry(0), m_iRetryPing(0)
{
...
void CDMARCMysql::SetOwnerName(char *_Name)
{	
	strcpy(mOwnerName, _Name);
}

//########################################## SetHost ###########################################
void CDMARCMysql::SetHost(const char *l_pszHost, const char *l_pszUser, const char *l_pszPasswd, const char *l_pszDBName, ULONG l_ulPort)
{
...
if(mIsUseLock == true)
	...
	if (SQLConn == NULL)
	{
		gLog->Write("E F[CDMARCMysql::ConnectDB] D[connectDB Err] V[(%s)%d.%s]", mOwnerName, mysql_errno(&m_MySQL), mysql_error(&m_MySQL));
	}
	else
	{
#ifdef SET_EUCKR
...
//########################################## QueryDB ###########################################
bool CDMARCMysql::QueryDB(const char *l_pszQuery)
{
	//totoro SM_D pLog->Write("D F[CDMARCMysql::QueryDB] D[Start] V[Owner:%s, query:%s]", mOwnerName, l_pszQuery);
	...
	if (nRet >= 0)
	{
		...
		{
			gLog->Write("E F[CDMARCMysql::QueryDB] D[(%s)query:%s] E[] V[%s.%d][mysql_real_query] error(%s)(%d)", mOwnerName, l_pszQuery, __FILE__, __LINE__, mysql_error(&m_MySQL), mysql_errno(&m_MySQL));
			...
			{
				gLog->Write("E F[CDMARCMysql::QueryDB] D[(%s)query:%s] E[ReconnectDB2 Failed]", mOwnerName, l_pszQuery);
				//exit(0);  //--ischoi--0630 임시
				return false;
			}
			else
			{
				gLog->Write("D F[CDMARCMysql::QueryDB] D[(%s)query:%s] E[ReconnectDB2 OK]", mOwnerName, l_pszQuery);
			}
			...
			{
				gLog->Write("E F[CDMARCMysql::QueryDB] D[(%s)query:%s] E[] V[%s.%d][mysql_real_query2] error(%s)(%d)", mOwnerName, l_pszQuery, __FILE__, __LINE__, mysql_error(&m_MySQL), mysql_errno(&m_MySQL));
				//exit(0);  //--ischoi--0630 임시
				return false;
			}
		}
...
//########################################## SelectDB ##########################################
bool CDMARCMysql::SelectDB(const char *l_pszQuery, vector<string> &l_rvecResult, int *_pRowCnt)
{
	int  iResult(0);
	int  iFields;
	int  iRows;
	...
	if (nRet >= 0)
	{
		...
		{ // 성공
			...
			{
				gLog->Write("E F[CDMARCMysql::SelectDB] D[query:%s] E[] V[%s.%d][mysql_real_query] error(%s)(%d)", l_pszQuery, __FILE__, __LINE__, mysql_error(&m_MySQL), mysql_errno(&m_MySQL));
			}
		...
		{
			gLog->Write("E F[CDMARCMysql::SelectDB] D[query:%s] E[0 == iResult] [mysql_real_query]", l_pszQuery);
...
void CDMARCMysql::SelectDB_FreeResult()
{
	if(mpMySQLResult)
	{
...
bool CDMARCMysql::SelectDB_GetResult(const char *l_pszQuery, int *_pRowCnt, int _FieldsCnt)
{
	//totoro SM_D pLog->Write("D F[CDMARCMysql::SelectDB_GetResult] D[Start] V[Owner:%s, query:%s]", mOwnerName, l_pszQuery);
	...
	{
		gLog->Write("E F[CDMARCMysql::SelectDB_GetResult] E[MakeUTF8 failed] V[(%s)]", mOwnerName);
		return false;
	}

	if(mpMySQLResult != NULL)
	{
		gLog->Write("E F[CDMARCMysql::SelectDB_GetResult] E[mpMySQLResult != NULL] V[(%s)]", mOwnerName);
		//exit(0);  //--ischoi--0630 임시
		return false;
	}
	...
	if (iResult != 0) //0이성공
	{
		gLog->Write("E F[CDMARCMysql::SelectDB_GetResult] D[(%s)query:%s] E[0 == iResult] [mysql_real_query]", mOwnerName, l_pszQuery);
		...
		if(ReconnectDB2() == false)
		{
			gLog->Write("E F[CDMARCMysql::SelectDB_GetResult] D[(%s)query:%s] E[ReconnectDB2 Failed]", mOwnerName, l_pszQuery);
			//exit(0);  //--ischoi--0630 임시
			return false;
		}
		else
		{
			gLog->Write("D F[CDMARCMysql::SelectDB_GetResult] D[(%s)query:%s] E[ReconnectDB2 OK]", mOwnerName, l_pszQuery);
		}
	...
	if (mpMySQLResult == NULL)
	{
		gLog->Write("E F[CDMARCMysql::SelectDB_GetResult] D[(%s)query:%s] E[] V[%s.%d][mysql_real_query] error(%s)(%d)", mOwnerName, l_pszQuery, __FILE__, __LINE__, mysql_error(&m_MySQL), mysql_errno(&m_MySQL));
	...
	else
	{
		...
		if(_FieldsCnt != iFields)
		{
			...
			gLog->Write("E F[CDMARCMysql::SelectDB_GetResult] D[(%s)query:%s] E[_FieldsCnt(%d) != iFields(%d)]", mOwnerName, l_pszQuery, _FieldsCnt, iFields);
...
//	int    iDBErr(mysql_errno(&m_MySQL));
	...
	{
		//gLog->Write("D F[CDMARCMysql::ReconnectDB] ReconnectDB Err[%d] Retry Count Over & WaitTime[%ld]\n", iDBErr, ulTimeDiff);
		if (ulTimeDiff > 30000)
			m_iRetry = 0;
		else
			return false;
	...
	//gLog->Write("D F[CDMARCMysql::ReconnectDB] ReconnectDB Err[%d] Retry[%d]\n", iDBErr, m_iRetry);
	DisconnectDB();
	return ConnectDB();
}

bool CDMARCMysql::CheckAlive()
{
	bool bResult(true);
	int  nRet(0);
...
int CDMARCMysql::RowCount(const char *l_pszQuery)
{
	int iResult(0);
	int iRows(-1);
...
bool CDMARCMysql::CallProc(const char *l_pszQuery, vector<string> &l_rvecResult)
{
...
bool CDMARCMysql::Transaction()
{
	return QueryDB("START TRANSACTION");
}

bool CDMARCMysql::Commit()
{
	return QueryDB("COMMIT");
}

bool CDMARCMysql::Rollback()
{
	return QueryDB("ROLLBACK");
}

long long CDMARCMysql::GetSQLInsertID()
{
	return mysql_insert_id(&m_MySQL);
}

int CDMARCMysql::MakeUTF8(const char *l_pszIn, size_t l_SizeIn, char *l_pszOut, size_t l_SizeOut)
{
	int nRet(0);
...
int CDMARCMysql::MakeEUCKR(const char *l_pszIn, size_t l_SizeIn, char *l_pszOut, size_t l_SizeOut)
{
	int nRet(0);
	boost::shared_array<char> szInBuff = boost::shared_array<char>(new char[l_SizeIn + 1]);
...
int CDMARCMysql::MakeEUCKRCOPY(const char *l_pszIn, size_t l_SizeIn, char *l_pszOut, size_t l_SizeOut)
{
